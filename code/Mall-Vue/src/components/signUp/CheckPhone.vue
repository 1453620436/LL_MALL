<template>
  <div class="info-form">
    <Form ref="formValidate" :model="formValidate" :label-width="80" :rules="ruleValidate">
      <FormItem label="手机号" prop="telethone">
          <i-input v-model="formValidate.telethone" clearable size="large"  placeholder="请输入手机号"></i-input>
      </FormItem>
...
export default {
  ...
  methods: {
    ...
    getcheckNum () {
      if (this.formValidate.telethone.length === 11) {
    ...
    handleSubmit (name) { // 提交验证
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.$router.push({ path: '/SignUp/inputInfo', query: { telethone: this.formValidate.telethone } });
