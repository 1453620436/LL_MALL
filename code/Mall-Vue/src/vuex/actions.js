...
import {addAddress,updateAddress,deleteAddress,getAddress} from '@/apis/address';
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const data = {
        ...
        remarks: {
          goodAnalyse: 90,
          remarksTags: ['颜色可人', '实惠优选', '严丝合缝', '极致轻薄', '质量没话说', '比定做还合适', '完美品质', '正品行货', '包装有档次', '不容易发热',
            '已经买第二个', '是全覆盖'
          ],
...
//加载当前登录用的收货地址（const声明常量，let声明变量）
export const GetAddress = ({commit,rootState}) => {
 const loginIdCard = rootState.userInfo.idCard;
  if(loginIdCard){
    return new Promise((resolve, reject) => {
        getAddress(loginIdCard).then(response=>{
        const address = response.data;
        resolve(response.data);//将Promise对象的状态从“未完成”变为“成功”（即从Pending变为Resolved），在异步操作成功时调用，并将异步操作的结果，作为参数传递出去
      }).catch(error => {
        alert("操作出现异常，请稍后再试");
        console.log("用户未登陆");
        reject(error);//将Promise对象的状态从“未完成”变为“失败”（即从Pending变为Rejected），在异步操作失败时调用，并将异步操作报出的错误，作为参数传递出去
      });
    });
  }
};

//添加收货地址(commit实质上是store的一个实例,可以去操作、获取state)
export const AddAddress = ({commit,rootState},data)=>{
  const loginIdCard = rootState.userInfo.idCard;
  if(loginIdCard){
    return new Promise((resolve,reject)=>{
      data.idCard = loginIdCard;
      addAddress(data).then(response=>{
        if (response.code == "200") { //等于200表示成功
          resolve()
          return true;
        } else {
          reject(error)
...
export const DeleteAddress = ({commit},data)=>{
  return new Promise((resolve,reject)=>{
    deleteAddress(data).then(response=>{
      resolve();
    }).catch(error=>{
        console.log(error);
        reject(error);
...
//修改收货地址
export const UpdateAddress = ({commit},data)=>{
  return new Promise((resolve,reject)=>{
  ...
  return new Promise((resolve, reject) => {
    register(data).then(response => {
      if (response.code == '200') { //表示注册成功
        ...
        reject(error);//抛出异常,意味着操作失败（fulfilled: 意味着操作成功完成，pending: 初始状态，成功或失败状态）
        resolve(false);
      }
    }).catch(error => {
      console.log(error);
...
export const Login = ({commit}, data) => {
  return new Promise((resolve, reject) => {
    login(data.username.trim(), data.password.trim()).then(response => {
      ...
      //_this.changeLogin({ Authorization: response.data });
       resolve();
       //return true;
      /* } else {
        reject(error)
